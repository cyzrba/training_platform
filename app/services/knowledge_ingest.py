...
def build_plan(content: bytes, filename: str) -> IngestPlan:
    ...
    try:
        ...
        chunks = splitting.split_document(parsed)
        if not chunks:
...
async def create_doc_from_bytes(
    ...
    *,
    ...
    plan = build_plan(content, filename)
...
async def reparse_document(session: AsyncSession, doc: KnowledgeDoc) -> IngestResult:
    """重新读源文件并重切（幂等重建；改了切分参数就用它）。"""
    content, filename = await load_source(session, doc)
    return await create_doc_from_bytes(
        session,
        content=content,
        filename=filename,
        title=doc.title,
