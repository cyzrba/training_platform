...
| 删除表 | 任务下发 `publish_task / publish_task_target / publish_task_project`（**v0.4 已按需求加回，见 `docs/方案设计.md`**）、评审维度 `review_dimension`、积分称号 `point_rule / point_log / title_def / student_title`、单关提交 `stage_submission / stage_submission_file`、`review_dimension_score`、`project_completion_rule` |
...
- `project_module` 是**项目组成表**：教师建项目时从模块库挑选模块，也可以**自创新模块**，一条记录代表“某个项目中的一个模块”：
  - `module_name` 是模块名称（来自模板或自定义）；从模块库挑选时 `template_id` 有值，自定义模块为空；
  - `stage_no` 决定项目内的填写顺序；
...
```text
总数   = 该技能关联的项目数（project_skill），**且项目已经发布任务给该学生**、仍是 PUBLISHED
完成数 = 该学生已完成的关联项目数（按项目去重）—— 同样限定在上面这个集合里
progress = 完成数 ÷ 总数 × 100
...
- **分母只算"发了任务给这个学生"的项目**（v0.4 起）：项目在项目管理里"发布"只代表编辑完成，
  没发任务给学生就看不到，也就不能算进分母。发布 / 撤回任务时会自动重算受影响学生的技能进度。
- 状态由进度推导（0 未解锁 / 0 < 进度 < 100 已激活 / 100 已精通），**进度会降**：重新挑战失败
  改判、或者撤回了任务（分母变小后仍按新集合算）都会重算并清掉对应的达标时间戳；
  同一项目重挑战只计一次；技能前置 `skill_node_dependency` 决定“未解锁”提示。
...
> 配套建议：项目一旦有学生开始闯关，建议锁定模块结构（不允许删除或改关卡名称）；确需调整时用“新增模块 + 停用旧模块”，避免历史 `attempt_stage` 指向被改动的模块配置。
...
| skill_tree | 技能树四大体系 | tree_name（唯一）、status |
| skill_node | 技能点 | tree_id、node_name（唯一）、unlock_note |
...
| project_stage_template | 标准化模块库（七大模块默认配置） | stage_name（唯一）、default_required、default_weight、default_requirement |
| project_module | 项目模块组成（一条 = 项目中的一个模块，可自定义、数量不限） | project_id、template_id（可空）、module_name、stage_no、required、weight、requirement、accept_standard |
...
| student_project_pick | 「我的实训」清单（学生自己挑的项目；老师点名必修不写表，展示时实时并集） | student_id、project_id、sort_no |
...
| 能力 | 原表 | 说明 |
| --- | --- | --- |
| 任务下发 / 发布管控 | publish_task、publish_task_target、publish_task_project | ~~本期不做~~ **v0.4 已实现**（新增 `publish_task_job` 岗位范围）：任务 = **必修**，把项目点名要求目标班级 / 分组的学生完成；项目本身 `PUBLISHED` 即对全体学生可见（2026-09-19 口径） |
