...
DOMAINS = [
    ...
    ("D 实训项目", ["training_project", "project_stage_template", "project_module", "project_file"]),
    ("E 闯关过程与评审", ["file_asset", "student_project", "student_project_pick", "training_attempt", "attempt_stage", "attempt_stage_file",
    ...
    ("J 任务下发", ["publish_task", "publish_task_target", "publish_task_job", "publish_task_project"]),
]
...
TABLE_CN = {
    ...
    "project_file": "项目附件",
    "file_asset": "文件元数据", "student_project": "学生实训记录",
    "student_project_pick": "我的实训清单", "training_attempt": "闯关轮次",
    ...
    "publish_task": "发布任务", "publish_task_target": "任务目标班级/分组",
    "publish_task_job": "任务岗位范围", "publish_task_project": "任务包含的项目快照",
}
...
TABLE_PURPOSE = {
    ...
    "project_skill": "项目与技能点的多对多关系：完成关联项目即推进该技能进度（分母 = 该技能点关联的全部已发布项目）。",
    ...
    "project_file": "项目附件：报告模板、数据文件、说明文档、评分标准等与项目的关联（文件本体在对象存储，这里只存关联）。",
    "file_asset": "文件元数据（MinIO 对象）的登记表：提交附件、头像、封面、证书文件、导入名单等。",
    "student_project": "学生个体的实训记录（教师导入名单时生成），承载填写进度、成绩与完成状态。",
    "student_project_pick": "「我的实训」清单：学生自己从项目库里挑的项目；老师发任务点名（必修）不写本表，展示时实时并集。",
    ...
    "publish_task": "发布任务：把项目下发给目标班级/分组，作为这批学生的「必修」；项目发布后本身就对全体学生可见，任务不再决定可见性。",
    "publish_task_target": "任务目标：一条 = 一个全班目标或一个指定分组目标；不选分组即全班。",
    "publish_task_job": "任务的岗位范围：只用于筛选项目（training_project.job_id），不筛学生；表内没有记录 = 全部岗位。",
    "publish_task_project": "任务包含的项目快照：发布时定稿，之后新增的同岗位同层级项目不会自动混进来。",
}
...
COMMON = {
    ...
    "selected_at": "选择时间", "switched_at": "切换时间",
    "tree_name": "技能树名称（唯一）", "tree_id": "技能树 ID（skill_tree.id）",
    "node_name": "技能节点名称（唯一）", "node_id": "技能节点 ID（skill_node.id）",
    ...
    "stage_name": "关卡 / 模块名称（唯一）", "required": "是否必填",
...
OVERRIDE = {
    ...
    "student_skill.progress": "技能进度（0~100，= 完成数 ÷ 关联项目总数；分母 = 该技能点关联的全部已发布项目，与任务无关）",
...
ENUMS = [
    ...
    ("publish_task.status", "PENDING（待发布）/ PUBLISHED（已发布）/ CANCELLED（已撤回）"),
    ("publish_task_target.target_type", "CLASS（全班）/ GROUP（指定分组）"),
